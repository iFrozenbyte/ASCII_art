...
public class AsciiArtServer {
    public static final int PORT = 8888;

    private final HttpServer server;
    private final TextGraphicsConverter converter;

    public AsciiArtServer(TextGraphicsConverter converter) throws Exception {
        if (converter == null) {
            throw new IllegalArgumentException("Серверу нужно передать в конструктор объект-конвертер, но было передано null.");
        }
        this.converter = converter;
        this.converter.setMaxHeight(200);
        this.converter.setMaxWidth(200);
        this.converter.setMaxRatio(7);
